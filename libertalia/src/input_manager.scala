...
object InputManager {
    ...
    def getAdjacentDenPirates(player : Player) : Seq[String] = {
        ...
        return PlayerManager.getAdjacentPlayers(player.playerId).map( p => p.pirates.filter( pirate => pirate.state == PirateState.Den).map( pirate => p.playerId.toString + pirate.rank.toString)).flatten
    }
